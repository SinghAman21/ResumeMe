...
export default function ReviewFeedback({ mode }: ReviewFeedbackProps) {
        ...
        setFeedback([
          {
            ...
            score: 35,
            feedback:
              "Did you design this in MS Paint? Because it looks like you tried really hard... at making the most generic resume possible.",
          },
          {
            category: "Content Quality",
            score: 45,
            feedback:
              "Ah yes, 'detail-oriented' and 'team player' - truly groundbreaking qualities that no other candidate has ever claimed before.",
          },
          {
            category: "Skills Presentation",
            score: 50,
            feedback:
              "Proficient in Microsoft Office? Wow, you and literally everyone else who has touched a computer since 1995.",
          },
          {
  ...
  const getGlowColor = (score: number) => {
    if (score >= 80) return "shadow-[0_0_15px_rgba(52,211,153,0.3)]"
    if (score >= 60) return "shadow-[0_0_15px_rgba(251,191,36,0.3)]"
    ...
    return (
      <div className="flex flex-col items-center justify-center py-12">
        <div className="relative mb-6">
          <div className="h-16 w-16 animate-spin rounded-full border-4 border-zinc-800/50 border-t-zinc-400"></div>
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="relative overflow-hidden rounded-xl border border-zinc-800/50 bg-zinc-900/30 p-8 text-center backdrop-blur-sm"
      >
        ...
        <div className="relative z-10">
          <motion.div
            className={cn(
              ...
              mode === "genuine" ? "border-indigo-500/20" : "border-rose-500/20",
      ...
      <div className="grid gap-4 md:grid-cols-2">
        {feedback.map((item, index) => (
          <motion.div
            ...
            className="relative overflow-hidden rounded-xl border border-zinc-800/50 bg-zinc-900/30 p-5 backdrop-blur-sm"
          >
            <div className="absolute inset-0 bg-gradient-to-br from-zinc-800/10 to-transparent"></div>
            <div className="absolute inset-0 bg-[url('/noise.svg')] opacity-[0.03] mix-blend-soft-light"></div>
            ...
            <div className="relative z-10">
              ...
              <p className="mt-4 text-sm text-zinc-300">{item.feedback}</p>
              ...
              {item.improvement && (
                <motion.div
                  ...
                  className="mt-3 flex items-start rounded-md bg-zinc-800/30 p-3"
                >
                  <Award className="mr-2 mt-0.5 h-4 w-4 flex-shrink-0 text-indigo-400" />
                  <p className="text-xs text-zinc-300">{item.improvement}</p>
                </motion.div>
              )}
            </div>
          </motion.div>
